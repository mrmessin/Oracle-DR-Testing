...
then
   echo "ERROR -> Script Prodtection on Please remove file .dr_end_protection and re-execute if you really want to run process"
   echo "ERROR -> Script Prodtection on Please remove file .dr_end_protection and re-execute if you really want to run process" >> ${LOG}
   exit 8
fi

echo "#################################################################################################"
echo "#################################################################################################" >> ${LOG}
echo "# Staring Process to end DR Testing Mode for database list ${inputfile}"
echo "# Staring Process to end DR Testing Mode for database list ${inputfile}" >> ${LOG}
echo "#################################################################################################"
echo "#################################################################################################" >> ${LOG}
...
do
   ...
   echo "Verifying Database ${dbname} is in Snapshot Standby Mode Prior to Attempting to Convert Back to Physical Standby."
   echo "Verifying Database ${dbname} is in Snapshot Standby Mode Prior to Attempting to Convert Back to Physical Standby." >> ${LOG}
   ...
   if [ "${result}" != "SNAPSHOT STANDBY" ]
    then
       echo "Database Instance ${instname} on ${nodename} is not in Snapshot Standby Mode, can not continue with DR Test Mode End, Skipping database"
       echo "Database Instance ${instname} on ${nodename} is not in Snapshot Standby Mode, can not continue with DR Test Mode End, Skipping database" >> ${LOG}
   else
      ...
      echo "After Database Restart Verify Database -> ${dbname} is a Physical Standby"
      echo "After Database Restart Verify Database -> ${dbname} is a Physical Standby" >> ${LOG}
      export cmd="export ORACLE_HOME=${ORACLE_HOME}; export ORACLE_SID=${instname}; echo -ne 'set feedback off\n set head off\n set pagesize 0\n select database_role from v\$database;' | $ORACLE_HOME/bin/sqlplus -s '/ AS SYSDBA'"
      export result=`ssh -n ${nodename} ${cmd} `
      #echo "${result}" >> ${LOG}
      #echo "${result}"
      ...
      if [ "${result}" != "PHYSICAL STANDBY" ]
       then
          echo "Database Instance ${instname} on ${nodename} is not in Physical Standby Mode, DR Test Mode End Failed."
          echo "Database Instance ${instname} on ${nodename} is not in Physical Standby Mode, DR Test Mode End Failed." >> ${LOG}
          exit 8
      fi

      echo "Starting Services for Database -> ${dbname}" 
      echo "Starting Services for Database -> ${dbname}"  >> ${LOG}
      # If not services supplied we default all services
      if [ "${services}" = "" ]
       then
