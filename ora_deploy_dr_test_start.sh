...
then
   echo "ERROR -> Script Protection on Please remove file .dr_start_protection and re-execute if you really want to run process"
   echo "ERROR -> Script Protection on Please remove file .dr_start_protection and re-execute if you really want to run process" >> ${LOG}
   exit 8
fi

echo "#######################################################################################"
echo "# Starting DR Testing Mode for List of databases to start DR Testing"
echo "#######################################################################################"
echo "#######################################################################################" >> ${LOG}
echo "# Starting DR Testing Mode for List of databases to start DR Testing" >> ${LOG}
echo "#######################################################################################" >> ${LOG}
...
do
   ...
   echo "Checking that database -> ${dbname} is a physical standby......."
   echo "Checking that database -> ${dbname} is a physical standby......." >> ${LOG}
   ...
   echo "Verify Database is in Snapshot Standby Mode for -> ${dbname}"
   echo "Verify Database is in Snapshot Standby Mode for -> ${dbname}" >> ${LOG}
   export cmd="export ORACLE_HOME=${ORACLE_HOME}; export ORACLE_SID=${instname}; echo -ne 'set feedback off\n set head off\n set pagesize 0\n select database_role from v\$database;' | $ORACLE_HOME/bin/sqlplus -s '/ AS SYSDBA'"
   export result=`ssh -n ${nodename} ${cmd} `
   #echo "${result}" >> ${LOG}
   ...
   echo "Verifying that Snapshot Standby is open READ WRITE -> ${dbname}"
   echo "Verifying that Snapshot Standby is open READ WRITE -> ${dbname}" >> ${LOG}
   ...
   echo "Startup All Configured Instances of Database -> ${dbname}...."
   echo "Startup All Configured Instances of Database -> ${dbname}...." >> ${LOG}
   ...
   #########################################################
   # Verify that it is snapshot standby mode
   echo "Verify post restart that database is in Snapshot Standby Mode -> ${dbname}"
   echo "Verify post restart that database is in Snapshot Standby Mode -> ${dbname}" >> ${LOG}
   export cmd="export ORACLE_HOME=${ORACLE_HOME}; export ORACLE_SID=${instname}; echo -ne 'set feedback off\n set head off\n set pagesize 0\n select database_role from v\$database;' | $ORACLE_HOME/bin/sqlplus -s '/ AS SYSDBA'"
   export result=`ssh -n ${nodename} ${cmd} `
   #echo "${result}" >> ${LOG}
   ...
   #########################################################
   # verify that the snapshot standby is open read write
   echo "Verify post restart that database is in READ WRITE Mode -> ${dbname}"
   echo "Verify post restart that database is in READ WRITE Mode -> ${dbname}" >> ${LOG}
   export cmd="export ORACLE_HOME=${ORACLE_HOME}; export ORACLE_SID=${instname}; echo -ne 'set feedback off\n set head off\n set pagesize 0\n select open_mode from v\$database;' | $ORACLE_HOME/bin/sqlplus -s '/ AS SYSDBA'"
   export result=`ssh -n ${nodename} ${cmd} `
   #echo "${result}" >> ${LOG}
   ...
   # Sleep for 5 seconds to allow services to register with listener
   sleep 5
    ...
    then
      ...
      if [ "${servicelist}" = "" ]
       then
         ...
         do
            # rcm does not use domain name in the service so no need to add domain for rcm
            if [[ ${instname,,} == *"rcmprd"* ]]; then
               echo ""
            else
   ...
   if [[ ${instname,,} == *"aries"* ]]; then
      echo "----------------------------------------------------------------------------------------------"
      echo "----------------------------------------------------------------------------------------------" >> ${LOG}
      echo "ARIES Database Truncating the work_queue, work_queue_rt and aggregation tables."
      echo "ARIES Database Truncating the work_queue, work_queue_rt and aggregation tables." >> ${LOG}

      export cmd="export ORACLE_HOME=${ORACLE_HOME}; export ORACLE_SID=${instname}; echo -ne 'set echo off\n truncate table ariestech.work_queue; \n truncate table ariestech.work_queue_rt; \n truncate table ariestech.aggregation;' | $ORACLE_HOME/bin/sqlplus -s '/ AS SYSDBA'"
      ssh -n ${nodename} ${cmd} >> ${LOG}
      #echo "${result}" >> ${LOG}

      if [ $? -eq 0 ]
       then
          echo "Aries Truncate of work_queue,work_queue_rt and aggregation OK......"
          echo "Aries Truncate of work_queue,work_queue_rt and aggregation OK......" >> ${LOG}
      else
          echo "ERROR -> Aries Truncate of work_queue,work_queue_rt and aggregation Failed......"
          echo "ERROR -> Aries Truncate of work_queue,work_queue_rt and aggregation Failed......" >> ${LOG}
      fi
   fi
   
   echo "Completed DR Test Mode Services Check for ${nodename} - ${dbname} - ${instname}"
   echo "Completed DR Test Mode Services Check for ${nodename} - ${dbname} - ${instname}" >> ${LOG}
   echo "----------------------------------------------------------------------------------------------"
   echo "----------------------------------------------------------------------------------------------" >> ${LOG}
done < "${inputfile}"
